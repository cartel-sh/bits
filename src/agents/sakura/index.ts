		...
		switch (command) {
				...
				case 'end':
						try {
								...
								await message.reply(`Ended study session. Duration: ${durationMinutes} minutes.`);
						} catch (error) {
								await message.reply('Failed to end session. Please check the session ID and try again.');
						}
						break;
						...
						try {
								...
								if (sessions.length === 0) {
										...
										const sessionList = sessions.map(s => {
												const duration = s.duration ? `${Math.round(s.duration / 60000)} minutes` : 'Ongoing';
				...
				default:
						await message.reply('Unknown command. Available commands: start, end, sessions');
		}
});
